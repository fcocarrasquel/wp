	...
	class YITH_Frontend_Manager_Section_Orders extends YITH_WCFM_Section {
		...
		public function print_shortcode( $atts = array(), $content = '', $tag = '' ) {
			...
			if( ! empty( $_GET['order_status'] ) ){
				$atts['query_args']['status'] = $_GET['order_status'];
			}
			...
			if( ! empty( $_GET['search'] ) ){
				$order = wc_get_order( sanitize_text_field( $_GET['search'] ) );
				$orders = $order !== false ? array( $order ) : array();
			}

			$atts['orders'] = apply_filters( 'yith_wcmv_print_orders_list_shortcode', $orders, $atts['query_args'] );

            if( empty( $atts['orders'] ) && ! $is_search ) {
